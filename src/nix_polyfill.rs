//! Temporary polyfill for nix until nix has included Epoll in a stable release.
...
use nix::{
    errno::Errno,
    libc::{self, c_int},
    sys::epoll::{EpollCreateFlags, EpollFlags, EpollOp},
};
...
impl Epoll {
    ...
    pub fn new(flags: EpollCreateFlags) -> nix::Result<Self> {
    ...
    pub fn add<Fd: AsFd>(&self, fd: Fd, mut event: EpollEvent) -> nix::Result<()> {
    ...
    #[allow(dead_code)]
    pub fn delete<Fd: AsFd>(&self, fd: Fd) -> nix::Result<()> {
    ...
    #[allow(dead_code)]
    pub fn modify<Fd: AsFd>(&self, fd: Fd, event: &mut EpollEvent) -> nix::Result<()> {
    ...
    pub fn wait(&self, events: &mut [EpollEvent], timeout: isize) -> nix::Result<usize> {
        let res = unsafe {
            libc::epoll_wait(
                self.0.as_raw_fd(),
    ...
    fn epoll_ctl<'a, Fd: AsFd, T>(&self, op: EpollOp, fd: Fd, event: T) -> nix::Result<()>
    where
